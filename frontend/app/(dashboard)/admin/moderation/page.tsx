...
export default function ModerationPage() {
  ...
  const [selectedReport, setSelectedReport] = useState<ModerationItem | null>(
    null,
  );
  const [actionType, setActionType] = useState<"delete" | "ignore" | null>(
    null,
  );
  const [filter, setFilter] = useState("pending");
  const [isLoading, setIsLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState("");
  const [actionMessage, setActionMessage] = useState<PageMessage | null>(null);
  ...
  useEffect(() => {
    const loadQueue = async () => {
      try {
        setIsLoading(true);
        setErrorMessage("");
        const items = await getModerationQueuePosts({
          threshold: REPORT_THRESHOLD,
        });
        const mapped: ModerationItem[] = (items || []).map(
          (post: QueuePost) => {
            const authorName =
  ...
  const handleAction = async (action: "delete" | "ignore") => {
    ...
    try {
      ...
      setActionMessage({
        ...
        text:
          action === "delete"
            ? "Post deleted and reported item resolved."
            : "Report ignored and item marked resolved.",
      });
    } catch (error: any) {
      setActionMessage({
        type: "error",
        text: error?.message || "Failed to apply moderation action",
      });
    } finally {
  ...
  const filteredReports =
    filter === "pending" ? pendingReports : resolvedReports;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {report.status === "resolved"
                                  ? (report as any).resolution === "deleted"
                                    ? "Deleted"
                                    : "Ignored"
                                  : "Pending"}
                              </Badge>
