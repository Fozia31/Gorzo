...
const posts: Post[] = [
  ...
  {
    id: 7,
    username: "FertilityHope",
    title: "My journey with fertility treatments",
    category: "Fertility",
    likes: 178,
    comments: 89,
  },
...
const feelingOptions = [
  {
    ...
    keywords: ["cramps", "pain", "menstrual"],
  },
  {
    label: "Worried about my cycle",
    keywords: ["cycle", "irregular", "period", "menstrual"],
  },
  { label: "Struggling with PCOS", keywords: ["pcos", "hormonal"] },
  {
    label: "Feeling anxious or stressed",
    keywords: ["anxiety", "stress", "mental", "depression"],
  },
  {
    label: "Questions about fertility",
    keywords: ["fertility", "pregnancy", "conception"],
  },
  {
    label: "Need nutrition advice",
    keywords: ["nutrition", "diet", "food", "weight"],
  },
];

function getRecommendations(message: string): {
  doctors: Doctor[];
  posts: Post[];
  response: string;
} {
  ...
  if (
    ...
    recommendedDoctors = doctors
      .filter((d) => d.specialty === "Mental Health")
      .sort((a, b) => b.rating - a.rating);
    recommendedPosts = posts
      .filter(
        (p) =>
          p.category === "Mental Health" ||
          p.title.toLowerCase().includes("anxiety") ||
          p.title.toLowerCase().includes("stress"),
      )
      .sort((a, b) => b.likes - a.likes)
      .slice(0, 3);
    response =
      "I understand you're going through a difficult time. It's completely normal to feel this way, and I'm here to help. Based on what you've shared, I recommend connecting with our mental health specialist who can provide personalized support.";
  }
  // Check for menstrual/cycle related keywords
    ...
    recommendedDoctors = doctors
      .filter((d) => d.specialty === "Gynecologist")
      .sort((a, b) => b.rating - a.rating);
    recommendedPosts = posts
      .filter(
        (p) =>
          p.category === "Menstrual Health" ||
          p.category === "Pain Management" ||
          p.title.toLowerCase().includes("cycle") ||
          p.title.toLowerCase().includes("cramp"),
      )
      .sort((a, b) => b.likes - a.likes)
      .slice(0, 3);
    response =
      "Menstrual health is so important, and I'm glad you're reaching out! Many sisters in our community have shared similar experiences. Here are some highly-rated resources and doctors who specialize in this area.";
  }
  // Check for PCOS related keywords
  else if (
    lowerMessage.includes("pcos") ||
    lowerMessage.includes("polycystic") ||
    lowerMessage.includes("hormonal") ||
    lowerMessage.includes("hormone")
  ) {
    recommendedDoctors = doctors
      .filter(
        (d) => d.specialty === "Gynecologist" || d.specialty === "Nutritionist",
      )
      .sort((a, b) => b.rating - a.rating);
    recommendedPosts = posts
      .filter(
        (p) =>
          p.category === "Conditions" ||
          p.title.toLowerCase().includes("pcos") ||
          p.title.toLowerCase().includes("hormonal"),
      )
      .sort((a, b) => b.likes - a.likes)
      .slice(0, 3);
    response =
      "PCOS can feel overwhelming, but you're not alone! Many women in our community have successfully managed their symptoms with the right support. I recommend speaking with both a gynecologist and nutritionist for a comprehensive approach.";
  }
  // Check for fertility related keywords
    ...
    lowerMessage.includes("conception") ||
    lowerMessage.includes("baby") ||
    lowerMessage.includes("trying to conceive")
  ) {
    recommendedDoctors = doctors
      .filter(
        (d) =>
          d.specialty === "Reproductive Health" ||
          d.specialty === "Gynecologist",
      )
      .sort((a, b) => b.rating - a.rating);
    recommendedPosts = posts
      .filter(
        (p) =>
          p.category === "Fertility" ||
          p.category === "Relationships" ||
          p.title.toLowerCase().includes("fertility") ||
          p.title.toLowerCase().includes("pregnancy"),
      )
      .sort((a, b) => b.likes - a.likes)
      .slice(0, 3);
    response =
      "Your fertility journey is deeply personal, and I'm here to support you. Our reproductive health specialists have helped many women achieve their dreams of motherhood. Here are some resources that might help.";
  }
  // Check for nutrition related keywords
    ...
    recommendedDoctors = doctors
      .filter((d) => d.specialty === "Nutritionist")
      .sort((a, b) => b.rating - a.rating);
    recommendedPosts = posts
      .filter(
        (p) =>
      ...
      .slice(0, 3);
    response =
      "Good nutrition is the foundation of women's health! Our nutritionist specializes in women's dietary needs and can create a personalized plan for you. Here are some popular resources from our community.";
  }
  // Check for pain related keywords
    ...
    recommendedPosts = posts
      .filter(
        (p) =>
          p.category === "Pain Management" ||
          p.title.toLowerCase().includes("pain") ||
          p.title.toLowerCase().includes("cramp") ||
          p.title.toLowerCase().includes("remedies"),
      )
      .sort((a, b) => b.likes - a.likes)
      .slice(0, 3);
    response =
      "I'm sorry to hear you're experiencing pain. It's important to address this with proper care. Many sisters have found relief through various methods shared in our community. Let me also recommend some specialists who can help.";
  }
  // Default response
  else {
    recommendedDoctors = doctors
    ...
    response =
      "Thank you for sharing! I'm here to help you find the right support. Based on our most popular resources and highest-rated doctors, here are some recommendations that might interest you. Feel free to tell me more about how you're feeling for more personalized suggestions.";
  }
...
const mapArticleToUi = (article: any): UiArticle => ({
  ...
  summary: String(
    article?.summary || article?.transcript || article?.textContent || "",
  ).slice(0, 140),
});
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello, dear sister! I'm your EFOY health companion. How are you feeling today? Share what's on your mind, and I'll recommend the best doctors and community discussions to support you.\n\nYou can tell me about any health concerns, or choose from the options below.",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSend = (text?: string) => {
    ...
    setTimeout(() => {
      const {
        ...
        posts: recPosts,
        response,
      } = getRecommendations(messageText);
              ...
              <div key={message.id} className="space-y-3">
                ...
                {message.recommendations && (
                  <div className="ml-11 space-y-4">
                    {/* Recommended Doctors */}
                    {message.recommendations.doctors &&
      ...
      {messages.length <= 2 && (
        <div className="mt-4 space-y-2">
          <p className="text-xs text-muted-foreground text-center">
            Quick options - Tell me how you&apos;re feeling:
          </p>
